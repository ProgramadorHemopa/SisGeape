...
{
    public class RelatorioServidorAposentadoriaEmExcel : FuncoesGeracaoExcel
    {
        List<string> Cabecalhos;
        List<FuncionarioRelatorioAposentadoriaModelView> Dados;
        CamposFuncionarioBuscaRelatorioModelView Campos;
        public RelatorioServidorAposentadoriaEmExcel(List<string> cabecalhos, List<FuncionarioRelatorioAposentadoriaModelView> dados, CamposFuncionarioBuscaRelatorioModelView campos)
        {
        ...
        public byte[] CriarRelatorio()
        {
            ...
            ISheet sheet = workbook.CreateSheet("Relatório Servidores");
            int rowNumer = 0;
            ...
            CreateCell(ref cell, "RELATÓRIO DE SERVIDORES POR APOSENTADORIA", false);

            Mesclar(ref sheet, new CellRangeAddress(rowNumer, rowNumer, 0, 5));
            ...
            foreach (var item in Dados.Select((it, value) => new { Conteudo = it, Index = value }))
            {
                ...
                CreateCell(ref cell, item.Conteudo.SITUACAO_NOME, true, WrapText(workbook));

                cell = row.CreateCell(5);
                CreateCell(ref cell, item.Conteudo.TIPO_APOSENTADORIA, true, WrapText(workbook));

                cell = row.CreateCell(6);
                CreateCell(ref cell, item.Conteudo.ENTRADA_APOSENTADORIA.HasValue ? item.Conteudo.ENTRADA_APOSENTADORIA.Value.ToShortDateString() : " - ", true, CenterStyle(workbook));
