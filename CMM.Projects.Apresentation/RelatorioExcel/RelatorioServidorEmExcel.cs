...
{
    public class RelatorioServidorEmExcel : FuncoesGeracaoExcel
    {
        List<string> Cabecalhos;
        List<FuncionarioRelatorioModelView> Dados;
        CamposFuncionarioBuscaRelatorioModelView Campos;
        public RelatorioServidorEmExcel(List<string> cabecalhos, List<FuncionarioRelatorioModelView> dados, CamposFuncionarioBuscaRelatorioModelView campos)
        {
        ...
        public byte[] CriarRelatorio()
        {
            ...
            CreateCell(ref cell, "RELATÓRIO DE SERVIDORES", false);
            ...
            foreach (var item in Dados.Select((it, value) => new { Conteudo = it, Index = value }))
            {
                ...
                CreateCell(ref cell, item.Conteudo.FUN_DATA_ADMISSAO.HasValue ? item.Conteudo.FUN_DATA_ADMISSAO.Value.ToShortDateString() : " - ", true, CenterStyle(workbook));

                cell = row.CreateCell(5);
                CreateCell(ref cell, item.Conteudo.FUN_DATA_DEMISSAO.HasValue ? item.Conteudo.FUN_DATA_DEMISSAO.Value.ToShortDateString() : " - ", true, CenterStyle(workbook));
            ...
            //Tamanho das colunas
            int[] TamanhoColuna = new int[] { 15, 40, 30, 35, 15, 15 };
