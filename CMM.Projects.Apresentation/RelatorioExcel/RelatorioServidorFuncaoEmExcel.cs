...
{
    public class RelatorioServidorFuncaoEmExcel : FuncoesGeracaoExcel
    {
        List<string> Cabecalhos;
        List<FuncionarioRelatorioFuncaoModelView> Dados;
        CamposFuncionarioBuscaRelatorioModelView Campos;
        public RelatorioServidorFuncaoEmExcel(List<string> cabecalhos, List<FuncionarioRelatorioFuncaoModelView> dados, CamposFuncionarioBuscaRelatorioModelView campos)
        {
        ...
        public byte[] CriarRelatorio()
        {
            ...
            ISheet sheet = workbook.CreateSheet("Relatório Servidores por Função");
            int rowNumer = 0;
            ...
            CreateCell(ref cell, "RELATÓRIO DE SERVIDORES POR FUNÇÃO", false);
            ...
            foreach (var item in Dados.Select((it, value) => new { Conteudo = it, Index = value }))
            {
                ...
                CreateCell(ref cell, item.Conteudo.FUNCAO, true, WrapText(workbook));

                cell = row.CreateCell(5);
                CreateCell(ref cell, item.Conteudo.FUNCAO_DATA_INICIO.HasValue ? item.Conteudo.FUNCAO_DATA_INICIO.Value.ToShortDateString() : " - ", true, CenterStyle(workbook));

                cell = row.CreateCell(6);
                CreateCell(ref cell, item.Conteudo.FUNCAO_DATA_FIM.HasValue ? item.Conteudo.FUNCAO_DATA_FIM.Value.ToShortDateString() : " - ", true, CenterStyle(workbook));
