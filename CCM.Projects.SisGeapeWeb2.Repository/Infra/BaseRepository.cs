  ...
  /// <typeparam name="TU">The database object type</typeparam>
    public class BaseRepository<T> : IBaseRepository<T> where T : class
    {
        ...
        public IEnumerable<T> GetPagedRecords(Expression<Func<T, bool>> whereCondition, Expression<Func<T, string>> orderBy, int pageNo, int pageSize)
        {
            ...
            return (dbSet.Where(whereCondition).OrderBy(orderBy).Skip(pageNo * pageSize).Take(pageSize)).AsEnumerable();
        }

        public IEnumerable<T> GetAllWithPagedRecords(Expression<Func<T, string>> orderBy, int pageNo, int pageSize)
        {
            return (dbSet.OrderBy(orderBy).Skip(pageNo).Take(pageSize)).AsEnumerable();
        }
        ...
        public async Task<IEnumerable<T>> ExecQueryAsync(string query)
        {
            return await dbSet.SqlQuery(query).ToListAsync();
        }
        ...
        {
            return await this._unitOfWork.Db.SaveChangesAsync();
        }
    }
}
